...
/* *Документация*
    Термины:
    - GLOBLA audio = аудио что звучат по всему миру, не являются 3D звуками, их располажение не влияет не на что.
        Пример: фоновая музыка, UI звуки при выборе/подтверждении/изменении пункта меню.
    - WOLRD audio = аудио что звучат локально, по идеи это 3D звуки их располажение влияет на их слышамость для Игрок(AudioListener)
        Пример: Звук выстрела/взырова/подбора бонуса/езды
        
    Ответственность класса(class responisbility):
    - 
    - 
    
    Потенциал развития:
    - сделать AudioManager GameEventListener или что-то такое - что бы выкинуть жосткую связанность кода
        через publis static final INSTANCE
    - при потребности сделать timer|cooldown для GLOBLA audio - что бы не было спама звука
    - ? вынести работу с WOLRD audio куда-то, тут оставить только код для GLOBLA audio
    - проверка что все зарание прописанные в коде Аудио-Ассеты МОГУТ быть успешно загружены(файлы рили существуют и норм грузятся).
...
public class AudioManager : MonoBehaviour
{
    ...
    public AudioSource GetOrLoad(GlobalAudioAssetDefinition definition)
    {
        var audio = allGlobalAudio[definition];
        return (audio != null) ? audio : this.AddAudioToGlobal(definition);
    }

    public AudioSource Play(GlobalAudioAssetDefinition definition, float delay = 0f)
    {
